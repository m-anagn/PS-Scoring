...
def main(args):
    ...
    # print('Start time:', str(datetime.datetime.now()), amino_acids, disp_threshold)
    ...
    if args.verbose:
        output.write('\t'.join( ['Protein ID', '\t'.join([aa_group + '_Composition' for aa_group in amino_acids]), 'Combined Linear Dispersion'] ) + '\n')
    else:
        output.write('\t'.join( ['Protein ID','Domain Sequence','Domain Boundaries','Final Domain Composition','Final Domain Linear Dispersion','\t'.join([aa for aa in all_aas]) ]) + '\n')
    
    proteome_size = get_proteome_size(args.fasta_file)
    ...
    for id, seq in fasta_parser(h):
        ...
        if len(trimmed_boundaries) > 0 and args.verbose==False:
            for i in range(len(trimmed_seqs)):
                output.write('\t'.join( [id, trimmed_seqs[i], trimmed_boundaries[i], str(round(final_comps[i], 2)), str(round(final_stddevs[i], 4)), '\t'.join([str(round(trimmed_seqs[i].count(aa) / len(trimmed_seqs[i]) * 100, 2)) for aa in all_aas]) ]) + '\n')
        elif len(trimmed_boundaries) > 0 and args.verbose==True:
            output.write('\t'.join( [id, '\t'.join( ['_'.join([str(round(x, 2)) for x in group_comps[aa_group]]) for aa_group in amino_acids] ), '_'.join([str(round(x, 4)) for x in normed_stddevs]) ] ) + '\n')
        else:
            continue
...
def get_params( args ):
    ...
    disp_threshold = args.dispersion - 0.000001    # +/-0.000001 --> ADJUST VALUE FOR DOWNSTREAM FLOATING POINT COMPARISONS
    ...
    # print(amino_acids, comp_thresholds)
...
if __name__ == '__main__':
    import sys, argparse, datetime, math, statistics
